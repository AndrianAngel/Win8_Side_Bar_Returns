...
; ═══════════════════════════════════════════════════════════════════════════
;  FULLSCREEN DETECTION - FIXED VERSION
; ═══════════════════════════════════════════════════════════════════════════
Func IsFullscreenAppActive()
    If Not $bIgnoreFullscreen Then Return False ; Skip check if option is disabled
    ...
    ; Check if window covers the entire desktop (fullscreen)
    If $aWinPos[0] <= 0 And $aWinPos[1] <= 0 And _
       $aWinPos[2] >= @DesktopWidth And $aWinPos[3] >= @DesktopHeight Then
        
        ; Additional check: if it's a game or fullscreen app, it might have no title bar
        ; Using WinGetState instead of WinGetStyle (which doesn't exist)
        Local $iWinState = WinGetState($hWnd)
        
        ; Check if window has no title bar (simplified check)
        ; WinGetState returns bit flags - we can check if it's minimized/maximized
        If BitAND($iWinState, 8) Or $aWinPos[2] = @DesktopWidth Then ; 8 = maximized
            Return True
        EndIf
    EndIf
